# Agent Handoff: Implement Chord Promote System
...
实现 **Chord Promote/Demote 系统** — 用户选中音符后可以将其标注为一个和弦对象。

**完整设计文档在 `CHORD_SYSTEM_DESIGN.md`（项目根目录）。** 这是经过多轮讨论确定的最终设计，务必通读。

## 最关键的设计决策（不要改）

1. **音符是真实对象，和弦是标注层。** ChordData 通过 `noteIds: string[]` 引用 `clip.notes` 中的实际 Note 对象。Promote 不删除音符，Demote 只删 ChordData。

2. **不做实时被动检测。** 只有用户选中 ≥2 个音符时才跑一次 `detectWithFallback`，结果显示在 ChordTrack 区域（目前是空的 placeholder div）。不选中时什么都不显示。

3. **第一版范围极小。** 只做：
   - Promote（选中音符 → 点击 ChordTrack 上的 promote SVG 图标按钮 → 创建 ChordData）
   - Demote（选中和弦 → 点击 ChordTrack 上的 demote SVG 图标按钮 → 删除 ChordData）
   - 在 ChordTrack 区域显示已 promote 的和弦名
   - 在和弦成员音符右侧显示级数标签（R, b3, 5, b7 等）
   - 选中音符时在 ChordTrack 显示检测预览 + Promote 按钮
...
- **按钮用 SVG 图标，不写文字。** 例如 promote 用向上箭头图标，demote 用向下箭头图标，确认用对勾 SVG，取消用叉叉 SVG。
...
| `src/utils/chordAnalysis.ts` | 保留的 `detectWithFallback` — promote 时用来识别和弦 |
| `src/store/projectStore.ts` | Zustand store，你要加 promoteNotes/demoteChord |
...
3. **Store** (`projectStore.ts`) — promoteNotes, demoteChord；(`uiStore.ts`) — selectedChordId
4. **ChordTrack UI** — 在 PianoRoll 的 placeholder div 里：
   - 选中 ≥2 音符时：显示检测到的和弦名 + promote 图标按钮（SVG 向上箭头或类似）
   - 已 promote 的和弦：显示和弦条 + 和弦名，选中时显示 demote 图标按钮（SVG 向下箭头或类似）
   - 不选中时 / 选中 <2 音符时：空
...
- **promoteNotes 时 noteIds 必须按 pitch 排序，跟 voices 一一对应。** `notesToChordData` 内部按 pitch 排序，所以 noteIds 也要用排序后的顺序。
...
完成后手动测试：
1. 画 C E G B 四个音 → 全选 → 点击 ChordTrack 上的 promote 按钮 → ChordTrack 出现 "Cmaj7" → 音符右侧出现 R / 3 / 5 / 7
2. 在 ChordTrack 点击和弦条 → 所有成员音符高亮
3. 点击 demote 按钮 → 和弦消失，音符恢复为自由音符
4. Undo → 和弦恢复
5. 只选 1 个音 → ChordTrack 不显示 promote 按钮
